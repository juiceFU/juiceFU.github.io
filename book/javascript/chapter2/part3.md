...
1. 正常数值的时候是两个数的乘积，两个正数相乘、两个负数相乘结果都是正数，如果只有一个数值是负数，结果就是负数，乘积超过js数值的表示范围，则返回Infinity 或 -Infinity；
2. 如果一个操作数是NaN，则结果也是NaN；
3. infinity与0相乘，结果是NaN；
4. Infinity与非0的数相乘，结果是Infinity或-Infinity；
5. Infinity与Infinity相乘，结果也是Infinity
6. 如果有一个不是数值，则先调用Number()转成数值，再走上面的规则

- 除法

1. 正常数值的时候是两个数的除法计算，两个正数相除、两个负数相乘结果都是正数，如果只有一个数值是负数，结果就是负数，乘积超过js数值的表示范围，则返回Infinity 或 -Infinity
2. 如果一个操作数是NaN，则结果也是NaN；
3. infinity除以infinity，结果是NaN；
4. 0被0除，结果是NaN；
5. 非0的有限数被0除，结果是Infinity或-Infinity；
6. Infinity被非0数除，结果是Infinity或-Infinity；
7. 如果有一个不是数值，则先调用Number()转成数值，再走上面的规则


- 求模 

1. 两个都是数值，按正常的除法计算，返回除得的余数；
2. 如果被除数是无穷大值而除数是有限大的数值，则结果是NaN；
3. 如果被除数是有限大的数值而除数是0，结果是NaN；
4. Infinity除Infinity，结果是NaN；
5. 被除数是有限大的数值而除数的无穷大的值，结果是被除数；
6. 被除数是0，结果是0；
7. 有一个操作数不是数值，则后台调用Number()将其转化为数值，再走上面的规则；
...
8. -0 - +0 //+0
9. -0 - -0 //+0
10. 一个操作数是字符串、布尔值、null、undefined，则先用后台调用Number()将其转换为数值，然后再走上面的程序，如果转换结果为NaN，减法的结果是NaN；
11. 有一个操作数是对象，先用valueOf(),不行的话再用toString()；
...
- 两个都是数值，执行数进行比较；
- 两个数是字符串，比较两个字符串对应的字符编码值；
    
    例如："23" < "3"  //true

- 有一个是数值，则将另一个转换成数值，然后再比较；
- 有一个是对象，先用valueOf(),不行了再用toString()，然后再比较；
- 有一个是布尔值，则将其转换为数值，再比较
